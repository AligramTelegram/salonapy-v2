...
export async function GET(request: NextRequest) {
  ...
  const appointments = await prisma.appointment.findMany({
    ...
    select: { price: true, staff: { select: { id: true, name: true, color: true } } },
  })

  const map = new Map<string, { staffId: string; staffName: string; staffColor: string; count: number; revenue: number }>()
  for (const a of appointments) {
    if (!a.staff) continue
    const key = a.staff.id
    const entry = map.get(key) ?? { staffId: key, staffName: a.staff.name, staffColor: a.staff.color, count: 0, revenue: 0 }
    entry.count++
